...
export interface Plugin<A = any> extends RolldownPlugin<A> {
  ...
  /**
   * Run logic when the server is restarted or closed. The hook receives the
   * {@link ViteDevServer} instance and a `reason` that is `'restart'` when the
   * server is restarting and `'close'` when it is closing.
   *
   * The hooks are called before the server is torn down. Hooks can be async
   * functions and will be called in series. On a forced exit the cleanup is
